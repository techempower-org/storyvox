...
class SettingsRepositoryUiImpl(
     ...
     * or remove of one that's not present) skips the write. The sync
     * stamp still fires unconditionally so a deliberate "touch" by the
     * user after a sync conflict still pushes their intent forward.
     * The key is in [SYNC_ALLOWLIST] + [SYNC_KEY_TYPES] so favorites
     * ride the next InstantDB sync round to the user's other devices.
     */
